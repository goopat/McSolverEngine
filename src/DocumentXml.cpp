...
namespace VarSetExpressions = McSolverEngine::DocumentXml::VarSetExpressions;
using VarSetCatalog = VarSetExpressions::VarSetCatalog;
...
)
{
    ...
    for (const auto& binding : bindings) {
        ...
        const auto parameterBinding = VarSetExpressions::parseParameterBindingExpression(binding.expression, catalog);
        if (!parameterBinding.binding) {
            if (parameterBinding.externalReference) {
                messages.push_back(parameterBinding.error);
                if (parameterBinding.error.find(VarSetExpressions::VarSetExpressionUnsupportedSubsetCode)
                    != std::string::npos) {
        ...
        if (const auto defaultValueString =
                VarSetExpressions::getVarSetValueForBinding(catalog, constraint.parameterKey)) {
