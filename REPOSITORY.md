...
支持跨草图外部几何引用（`ExternalGeometry`）的级联更新：导入时解析拓扑命名引用（如 `Sketch002.;g3v2;SKT`）并递归导入依赖草图，求解时按拓扑顺序自动重解整条依赖链，再通过草图平面间 2D 仿射变换刷新外部几何投影，无需新增公开 API（详细设计见 `docs/cross_sketch_cascade_update_design.md`）。
...
│   ├── CompatSolver.cpp           #   兼容求解器（SketchModel → GCS → 结果，含级联前导）
│   ├── ConsoleShim.h              #   控制台输出垫片
│   ├── DocumentSolver.cpp/h       #   跨草图级联：外部几何更新 + 依赖拓扑排序（内部）
│   ├── DocumentXml.cpp            #   Document.xml 自定义解析器（含 ExternalGeometry 链接解析）
│   ├── Engine.cpp                 #   引擎版本/描述
│   ├── GeometryExport.cpp         #   精确几何导出实现
│   ├── ParameterValueUtils.h      #   参数值校验工具
│   ├── PlaneTransform.cpp/h       #   草图平面间 2D 仿射变换（内部，纯线性代数）
│   ├── SketchShapeBuilder.cpp/h   #   草图形状 → OCCT TopoDS 构建
│   ├── SketcherGlobal.h           #   兼容垫片（SketcherExport → MCSOLVERENGINE_EXPORT）
│   ├── SubElementResolver.cpp/h   #   拓扑引用名（gNvM/Edge/Vertex）→ 源几何解析（内部）
...
│   ├── freecad_sources_test.cpp   #   FreeCAD Sketcher 求解器测试套件对照迁移
│   └── v111t003_debug.cpp         #   角度参数全范围扫描回归（V111.T001/T003、V101.T002 及 shortR 变体）
│
├── wrapper/                       # 跨语言包装层
...
│   ├── V101.Cascade.FCStd / V101.Cascade.xml（四草图外部几何级联链：Sketch←Sketch001←Sketch002←Sketch003，共享 XZ 平面，4 个 VarSet 参数驱动）
│   ├── V101.Cascade.solver.FCStd / V101.Cascade.solver.xml / V101.Cascade.solver.Sketch*.Shape.brp（四参数变更后的 FreeCAD 重算对照，四草图均严格比对）
│   ├── V111.7.100.FCStd / V111.7.100.brp / V111.7.100.solver.brp
│   ├── V111.10.FCStd / V111.10.xml（+ 5 参数覆盖，多解分支样本，BREP 对比非致命）
│   ├── V111.10.1892.FCStd / V111.10.1892.xml / V111.10.1892.Sketch*.Shape.brp（V111.10 参数 1892 覆盖的 FreeCAD 重算对照）
│   ├── V101.T002(.shortR).FCStd / V111.T001(.shortR).FCStd / V111.T003(.shortR).FCStd（表达式驱动角度约束全范围扫描语料）
...
│   ├── package_no_occt_dotnet.bat #   NoOcct + .NET 包装层变体快捷调用
│   ├── publish_nuget.ps1 / publish_noocct.bat / publish_debug_noocct.bat  # 发布辅助脚本（本地，未入库）
│   ├── McSolverEngine.targets     #   MSBuild 集成模板（C++ 原生包）
│   └── McSolverEngine_Net.targets #   MSBuild 集成模板（.NET 包装层包）
│
├── docs/                          # 设计文档
│   └── cross_sketch_cascade_update_design.md  # 跨草图外部几何级联更新设计
│
├── tools/                         # 构建工具
...
│
├── .pixi/                         # pixi conda 环境（Eigen3, libboost-headers, OCCT, VS 工具链）
...
| `McSolverEngineFreeCadSourcesTest` | 可执行文件 | FreeCAD 源码对照测试 |
| `McSolverEngineV111T003DebugTest` | 可执行文件 | 角度参数全范围扫描回归（0.01°–359.99°，六用例并发） |
...
2. **libboost-headers**: 仅查 `.pixi/envs/default/Library/include/`；若未找到，则直接配置失败
...
- 解析 `ExternalGeometry` 链接与拓扑命名引用（`ref="Sketch002.;g3v2;SKT"`），并递归导入被引用的依赖草图存入 `SketchModel`（`externalGeometryMap` 同时注册位置 id 与持久 id 两套映射）
...
- 级联前导：若模型含依赖草图，先按拓扑序（DFS 后序，带环检测）递归求解全部依赖草图，再由 `src/DocumentSolver.cpp` 用 `src/PlaneTransform.cpp` 的平面间 2D 仿射变换刷新本草图的外部几何投影（`src/SubElementResolver.cpp` 负责 `gNvM`/`Edge`/`Vertex` 引用名解析）；任一依赖求解失败则整体失败
...
| `McSolverEngine::DocumentXml` | XML 导入 |
| `McSolverEngine::BRep` | BREP 导出 |
| `McSolverEngine::Geometry` | 精确几何导出 |
| `McSolverEngine::ZipExtract` | ZIP/FCStd 提取 |
...
| 测试套件 | 文件 | 说明 |
|---|---|---|
| C++ SmokeTest | `tests/smoke.cpp` | 核心库回归：48+ 用例覆盖所有几何/约束类型、表达式引擎、参数化求解、Block、跨草图外部几何级联等 |
...
| FreeCAD 源码对照 | `tests/freecad_sources_test.cpp` | 从 FreeCAD Sketcher 求解器测试套件严格迁移的对照用例（几何坐标/约束类型/取值逐一复现） |
| 角度全范围扫描 | `tests/v111t003_debug.cpp` | V111.T001/T003、V101.T002 及 shortR 变体，表达式驱动 Angle 约束在 0.01°–359.99° 全范围（0.01° 步进）求解成功率回归，六用例并发 |
...
| Python Wrapper | `wrapper/python/tests/` | 28 个用例：求解、参数覆盖、BREP、几何导出、FCStd 提取、SVG 输出、版本、V111.9 等价校验、V111.T003 角度参数 |
...
目前已知最容易复现的是 `V111.9.xml + VarSet.L1=500`，而对 `V111.9.500.xml` 的 FreeCAD 对照测试也发现同类随机性。因此，C++ 与 Python 针对这两条 `V111.9` 相关用例做了**定向放宽**：只放宽几何对象在 `.brp` 文本中的位置，不再要求对象记录顺序严格一致；但几何对象本身的数据内容仍然必须在既有数值容差范围内严格匹配。除 `V111.9` 的顺序漂移特例与下文 `V111.10` 的多解分支特例外，其余 BREP 回归样例仍保持严格 token 顺序校验。

**V101.Cascade 级联回归说明**

`V101.Cascade.xml` + 四个 VarSet 参数（`arc_center_height=20000`、`arc_angle_001=90`、`arc_angle_003=150`、`height_2=10000`）的回归覆盖完整 参数 → 级联 → 求解 链路：期望 BREP 提取自 `V101.Cascade.solver.FCStd`（FreeCAD 对四参数变更后的重算结果），Sketch/Sketch001/Sketch002/Sketch003 四张草图全部严格 token 比对（实测最大浮点尾差 ≤ 4e-10），另附 Sketch003 级联外部点坐标的显式断言。注意 Sketch003 内部折线链存在多解分支，当前期望数据是在 FreeCAD 中重解到与本引擎相同分支后的状态；若 FreeCAD 文档未来从另一分支状态保存，该用例会再次分叉（属真实解分支差异，非级联错误）。

**V111.10 多解分支说明**

`V111.10`（+5 参数覆盖）与 `V111.10.1892` 两组用例存在已知多解问题，FreeCAD 与本引擎可能收敛到不同解分支，BREP 对比为非致命（mismatch 仅告警不失败），与 V111.9 的顺序漂移放宽不同：这里放宽的是几何内容本身。

### 6.3 回归数据（fcstdDoc/）

`fcstdDoc/` 是权威的回归语料库，共 71 个受版本控制文件（含 25 个 `.FCStd`）：

- **样本 1-3**：基础单/多平面草图
- **V101.Cascade**：四草图外部几何级联链（Sketch←Sketch001←Sketch002←Sketch003），共享 XZ 平面，4 个 VarSet 参数驱动；`V101.Cascade.solver.*` 为参数变更后的 FreeCAD 重算对照
- **V101.T002 / V111.T001 / V111.T003**（含 `.shortR` 变体）：表达式驱动 Angle 约束的全范围角度扫描语料
- **V102.1**：参数化尺寸驱动
...
- **V102.8**：单草图 BREP 回归
- **V111.7.100**：参数化 BREP 求解
- **V111.9**：参数化 BREP 非确定性顺序漂移校验样本（含 V111.9.500 参数覆盖变体）
- **V111.10 / V111.10.1892**：多解分支样本（BREP 对比非致命；1892 为 5 参数覆盖的 FreeCAD 重算对照）
...
| 依赖 | 版本 | 类型 | 说明 |
|---|---|---|---|
| Eigen3 | 3.4.0 | header-only | 线性代数（通过 pixi 提供） |
| libboost-headers | 1.84.0 | header-only | Graph, Math constants（通过 pixi 提供） |
| OCCT | 7.8.1 | 动态库 | 可选，BREP 导出（通过 pixi 提供，*novtk*） |
| zlib | 1.3.2 | 源码 | 内置，仅 inflate 子集，符号隔离 |
| PlaneGCS | 1.1.1 | 源码 | 内置，从 FreeCAD 1.1.1 复制 |
| CMake | 3.22+ | 构建工具 | — |
...
| 外部几何级联范围 | 仅支持同一文档内草图之间的引用级联；`Frozen`/`Sync`/`Detached` 等 ExternalGeometryExtension 标志未实现，外部几何一律按最新依赖重算；依赖环会显式求解失败（不会栈溢出） |
...
| `docs/cross_sketch_cascade_update_design.md` | 跨草图外部几何级联更新设计（含真实 FreeCAD 1.0.1 文档适配与 V101.Cascade 回归说明） |
