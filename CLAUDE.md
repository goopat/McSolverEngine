...
**Test structure**:
- `tests/smoke.cpp` — 48+ regression cases: all geometry/constraint types, expression engine, parameterized solving, Block, BREP token comparison, cross-sketch cascade (V101.Cascade two-sketch + three-sketch chain, geometry type coverage for Point/LineSegment/Circle/Arc)
...
- `tests/unit.cpp` — unit tests: ParameterValueUtils, CompatModel API, Diameter/Equal/Parallel constraints, conflict/redundancy detection, ZIP/GeometryExport/XML edge cases, PlaneTransform, SubElementResolver, external geometry positional IDs
...
5. **Compat Model** — `CompatModel.h/cpp`. `SketchModel` is the central in-memory model. All geometry/constraint types are `std::variant` structs — no FreeCAD types. Includes `ExternalGeometrySource` (source object + sub-element mapping) and cross-sketch cascade context (`dependencyModels_`, `dependencySolveOrder_`, `hasDependencyCycle_` flag)
6. **Compat Solver** — `CompatSolver.h/cpp`. Translates `SketchModel` → GCS parameters → solves → writes results back. Handles `ParameterMap` overrides. **Four-level solver fallback chain**: DogLeg → LevenbergMarquardt → BFGS → SQP augmented (aligns with FreeCAD's `internalSolve`). Key params: `convergence=1e-10`, `maxIter=100`, `FullPivLU`. Cascade preamble: before solving a sketch, refresh external geometry from already-solved upstream dependencies
7. **Document.xml Import + Inspect** — `DocumentXml.h/cpp`. Custom XML parser (no external library). `importSketchFromDocumentXml` extracts geometries, constraints, Placement, ExpressionEngine, VarSet for solving. `importAllSketchesFromDocumentXml` builds a dependency graph from `ExternalGeometry` links, detects cycles, performs topological sort, and imports all sketches in solve order. `inspectDocumentXml` is a lightweight browse API returning sketch/varSet metadata including geometry element types, construction flags, constraint kinds, and cross-references without running the solver
7a. **Cross-sketch cascade** — `DocumentSolver.cpp/h` (external geometry update via `PlaneTransform`), `SubElementResolver.cpp` (topological-naming `gN`/`gNvM` → geometry index resolution), `PlaneTransform.cpp/h` (cross-plane 2D affine transform between sketch placements). These support the cascade pipeline: import all sketches → build dependency graph → topological sort → solve in order, refreshing external geometry from solved sources before each dependent solve
8. **VarSet Expression Engine** — `VarSetExpressionEngine.cpp/h`. Lightweight FreeCAD expression subset: arithmetic, comparison operators, ternary conditional (lazy selected-branch evaluation, matching FreeCAD's `ConditionalExpression`), trig/hyperbolic functions, pi/e constants, pow/sqrt/log, min/max/sum/average/count, limited length/angle unit conversion. Supports VarSet/Sketch property reference chains with cycle detection. Non-VarSet object references (e.g. `Spreadsheet.Width`) return `VarSetExpressionUnsupportedSubset` error
...
- **VarSet expression subset**: Arithmetic, comparisons (`==`/`!=`/`<`/`>`/`<=`/`>=`), ternary conditional `cond ? a : b` (only the selected branch must evaluate, matching FreeCAD's `ConditionalExpression`), trig/hyperbolic, pi/e, pow/sqrt/log, min/max/sum/average/count, limited unit conversion (length: mm/cm/m/km/um/nm/in/ft; angle: deg/rad). Supports `<<Label>>.Param` and `VarSetName.Param` references. No Spreadsheet refs, no full Quantity/Unit, no boolean logic operators — these return `VarSetExpressionUnsupportedSubset`
...
- **Parameter validation order**: In `solveSketch`, validate parameters (`tryParseApiParameters`) **before** any cascade logic. Invalid parameters must not trigger side effects (dependency solve, external geometry update) — this prevents "fail-but-mutated" behavior
- **Dependency cycle detection**: `importAllSketchesFromDocumentXml` uses DFS with an `imported` set (completed) for cycle pruning. `hasDependencyCycle_` flag on `SketchModel` causes `solveSketch` to return `Failed` immediately rather than attempting stale-geometry independent solve
- **PlaneTransform rotation decomposition**: Local axes are **column vectors** in world coordinates (`world = R × local`). `decomposePlacement` reads columns (not rows) of the rotation matrix. Using row vectors produces wrong results for non-standard rotations. The `referenceAngle()` function recovers the rotation angle via `atan2(m10, m00)` from the first column of the conformal matrix
- **Arc angle handling in transforms**: Arc angles (polar angles around center) are **not preserved** under general conformal transforms. Rotation: `start += θ, end += θ`. Reflection: `start = φ − srcEnd, end = φ − srcStart` (mirror + swap to maintain orientation). Conic arc angles (ellipse/hyperbola/parabola) are relative to the major axis, which transforms with the geometry — preserved under rotation, sign-flipped under reflection
