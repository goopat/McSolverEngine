...
- 当前原生回归仍以自定义 smoke test 可执行文件 + `CTest` 为主；另有 `wrapper\csharp\tests` 下的 net48 + net8.0 MSTest 项目，通过 `dotnet test` 回归托管包装层
...
完整的 C API 类型定义、函数签名、处理管线、参数约定、内存管理、线程安全和错误处理模式见 **[McSolverEngine_C_API.md](McSolverEngine_C_API.md)（英文版： **[McSolverEngine_C_API.en.md](McSolverEngine_C_API.en.md)**）。
...
8. `V102.7.FCStd` 提取 + 参数化 BREP 包装接口回归（C# / Python）
9. `V111.9.xml` / `V111.9.500.xml` BREP 几何等价包装接口回归（Python），允许对象顺序漂移
10. BREP 对照采用 token 级比较 + 数值容差，避免 OCCT 末位浮点抖动带来误报
...
3. **跨语言 C ABI 接口**
   ...
   - 详见 **[McSolverEngine_C_API.md](McSolverEngine_C_API.md)** （英文版： **[McSolverEngine_C_API.en.md](McSolverEngine_C_API.en.md)** ）
...
| 类型 | 状态 |
|---|---|
| Coincident, Symmetric, PointOnObject | 无直接测试（仅 XML 间接覆盖） |
